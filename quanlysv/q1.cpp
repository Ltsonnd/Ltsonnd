...
void tinhDTB(SV &sv) {
    sv.diemTB = 0.15*(sv.pt1+sv.pt2)+0.1* (sv.lab1+sv.lab2) + 0.05*(sv.pres1+sv.pres2) + 0.4*(sv.fe);    
}
...
void nhapThongTinSV(SV &sv, int id) {
	cout <<"\nNhap msv: "; fflush(stdin); gets(sv.msv);
    do {
	cout<<"\nNhap ten sinh vien: ";
    fflush(stdin); gets(sv.ten);
    if(KiemTraKyChu(sv.ten)==0)
    cout<<"\nNhap lai ten sinh vien: ";
    }while(KiemTraKyChu(sv.ten)==0);
    cout <<"Nhap gioi tinh: "; gets(sv.gioiTinh);
    cout <<"Nhap tuoi: "; cin >> sv.tuoi;
    cout<<"\nNhap diem progress test 1: ";cin >> sv.pt1;
    cout<<"\nNhap diem progress test 2: ";cin>> sv.pt2;
    cout<<"\nNhap diem lab1: ";cin>> sv.lab1;
    cout<<"\nNhap diem lab2: ";cin>> sv.lab2;
    cout<<"\nNhap diem presentation 1: ";cin>> sv.pres1;
    cout<<"\nNhap diem presentation 2: ";cin>> sv.pres2;
    cout<<"\nNhap diem final exam: ";cin>> sv.fe;
    sv.id = id;
    tinhDTB(sv);
    xeploai(sv);
}
...
void showStudent(SV a[], int n) {
    printLine(100);
    cout <<"\n\STT\tID\tMSV\tHo va ten\tGioi tinh\tTuoi\t pt1 \t pt2 \t Lab1 \t Lab2 \t pres1 \t pres2 \t FE \t Total\tDiem TB\t Hoc luc";
    for(int i = 0; i < n; i++) {
        ...
        printf("\t\t%d", a[i].tuoi);
        cout<<"\t "<< a[i].pt1 <<"\t "<< a[i].pt2 <<"\t "<< a[i].lab1<<"\t "<< a[i].lab2 <<"\t "<< a[i].pres1 <<"\t "<< a[i].pres2<<"\t "<< a[i].fe<<"\t "<<a[i].diemTB;
...
int docFile(SV a[], char fileName[]) {
    ...
    while (fscanf(fp, "%5d%30s%5s%5d%10f%10f%10f%10f%10s\n", &a[i].id, &a[i].msv,&a[i].ten, 
            &a[i].gioiTinh, &a[i].tuoi, &a[i].pt1, &a[i].pt2, &a[i].lab1,  &a[i].lab2, &a[i].pres1,&a[i].pres2,&a[i].fe,
...
void ghiFile(SV a[], int n, char fileName[]) {
    ...
    fp = fopen (fileName,"w");
    for(int i = 0;i < n;i++){
        fprintf(fp, "%5d%30s%5s%5d%10f%10f%10f%10f%10s\n", a[i].id,a[i].msv, a[i].ten,a[i].gioiTinh, 
            a[i].tuoi, a[i].pt1, a[i].pt2, a[i].lab1,  a[i].lab2, a[i].pres1,a[i].pres2,a[i].fe, a[i].diemTB, a[i].hocluc);
    }
    fclose (fp);
}
